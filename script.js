...
function BuatDepartement(namaDpr, docId){
  ...
  // contentLinkdp.dataset.docId = docId;
...
async function AddDocument_CustomID(){
  ...
  .then(() => {
      BuatDepartement(NameBox.value, RollBox.value);
      alert("sukses");
  })
  .catch((error) => {
  ...
  async function UpdateFieldsInADocument(docId, newValue) {
    ...
    .then(() => {
        ...
        cardElement.querySelector('h3').innerText = newValue;
                ...
                // Perbarui elemen di sidebar
        var sidebarElement = document.querySelector(`[data-doc-id="${docId}"]`).closest('.listdp');
        if (sidebarElement) {
            sidebarElement.querySelector('h3').innerText = newValue;
        }
...
async function DeleteDocument(docId, cardElement) {
  ...
  .then(() => {
      ...
      // Hapus elemen di sidebar
      var sidebarElement = document.querySelector(`#DpSideBar [data-doc-id="${docId}"]`);
      if (sidebarElement) {
